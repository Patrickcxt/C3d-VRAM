...
if opt.rnnPath ~= '' and opt.agentPath ~= '' and clsPath ~= '' then
    ...
    --rnn = torch.load("./all/detector/trained_rnn_40000.t7")
    --agent = torch.load("./all/detector/trained_agent_40000.t7")
    rnn = torch.load("./all/detector2/trained_rnn_10000.t7")
    agent = torch.load("./all/detector2/trained_agent_10000.t7")
    --rnn = torch.load("./sports/detector/trained_rnn_200000.t7")
    --agent = torch.load("./sports/detector/trained_agent_200000.t7")
    --rnn = torch.load("./sports/detector2/trained_rnn_1600.t7")
    --agent = torch.load("./sports/detector2/trained_agent_8000.t7")
end
...
if opt.clsPath ~= '' then
    ...
    --classifier = torch.load("./all/classifier/trained_classifier_40000.t7")
    --classifier = torch.load("./all/classifier2/trained_classifier_300000.t7")
    classifier = torch.load("./all/classifier3/trained_classifier_120000.t7")  -- trained on my ubuntu, no background
    --classifier = torch.load("./sports/classifier2/trained_classifier_300000.t7")
    --classifier = torch.load("./sports/classifier/trained_classifier_300000.t7")
    --classifier = torch.load("./sports/classifier3/trained_classifier_120000.t7")  -- trained on my ubuntu, no background
end
...
function demo()
    ...
    for i = 1, #valSet do
        ...
        for j = 1, #rnn_output do
            ...
            pred[j] = _clip_segments(output[1])
            local c3d, _, _ = dh:getSingleTrimedByName(valSet[randIdx[i]], pred[j])
            local cls_pred = classifier:forward(c3d)
            cls_pred = softmax:forward(cls_pred)
            local maxVal, maxId = torch.max(cls_pred, 1)
            --[[
            if maxVal[1] >= 0.5 and maxId[1] ~= 201 then
                print(">> Yes <<")
            ...
            local gst, ged = segment_target[1][1], segment_target[1][2]
            if segment_target:size(1) == 1 and ged-gst < 0.5 then
                local overlap = utils.interval_overlap(segment_target, pred[j]:resize(1, 2))
                if overlap[1][1] > 0.8 then
                    print(info)
                    print(segment_target)
                    print(pred[j])
                    print(label_target)
...
function test_net()
    ...
    for i = 1, #valSet do
        ...
        for j = 1, #rnn_output do
            ...
            local c3d, _, _ = dh:getSingleTrimedByName(vidName, seg_pred)
            local cls_pred = classifier:forward(c3d)
            cls_pred = softmax:forward(cls_pred)
            local maxVal, maxId = torch.max(cls_pred, 1)
...
function test_net_nms()
    ...
    for i = 1, #valSet do
        ...
        for j = 1, #rnn_output do
            local output = agent:forward(rnn_output[j])
            seg_pred = _clip_segments(output[1]):resize(2)
            local c3d, _, _ = dh:getSingleTrimedByName(valSet[randIdx[i]], seg_pred, 3)
            local cls_pred = classifier:forward(c3d)
            cls_pred = softmax:forward(cls_pred)
            local maxVal, maxId = torch.max(cls_pred, 1)
            if maxVal[1] >= 0.3 and maxId[1] ~= 201 then
                table.insert(seg_pred_set, {seg_pred[1], seg_pred[2]})
                table.insert(cls_pred_set, maxId[1])
                table.insert(scores, maxVal[1])
            end
