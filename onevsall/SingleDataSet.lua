...
function SingleDataSet:__init(rootPath, idxPath, jsPath) -- /path/to/your/Videos/
    ...
    self.c3dPath = "/media/chenxt/906837F86837DC22/ActivityNet/sub_activitynet_v1-3.c3d.hdf5"
...
function SingleDataSet:setClass(clsName, mode)
    ...
    self.posTrimed, self.negTrimed = self:get_trimed_dataset(self.mode)
...
function SingleDataSet:separateOneVsAll(clsLabel)
    local subPosSet, subNegSet = {}, {}
    local dataset = nil
    ...
    for i = 1, #dataset do
        local cutName = self:cut_name(dataset[i])
        ...
        local vid = {}
        vid['name'] = dataset[i]
        vid['duration'] = vidInfo['duration']
        vid['gt_segments'] = vidInfo['gt_segments']

        local fix_labels = torch.Tensor():resizeAs(gt_labels)
        local flag = false
        for j = 1, gt_labels:size(1) do
            if self.all2sub[gt_labels[j]] == clsLabel then
                flag = true  -- Positive sample
                fix_labels[j] = 1
            else
                fix_labels[j] = -1
            end
        end
        vid['gt_labels'] = fix_labels
        if flag then
            table.insert(subPosSet, vid)
        else
            table.insert(subNegSet, vid)
        end
    end
    print(#subPosSet)
    print(#subNegSet)
	self.posIdx, self.negIdx = 1, 1
    if self.mode == 'training' then
        self.subPosTrainSet, self.subNegTrainSet = subPosSet, subNegSet
    else
        self.subPosValSet, self.subNegValSet = subPosSet, subNegSet
    end
    return subPosSet, subNegSet
end
--[=[
function SingleDataSet:separateOneVsAll(clsLabel)
    local subPosTrainSet, subNegTrainSet = {}, {}
    for i = 1, #self.subTrainSet do
        local cutName = self:cut_name(self.subTrainSet[i])
        ...
        vid['gt_labels'] = fix_labels
        if flag then
            table.insert(subPosTrainSet, vid)
        else
            table.insert(subNegTrainSet, vid)
        end
    end
    print(#subPosTrainSet)
    print(#subNegTrainSet)
    io.read()
	self.posIdx, self.negIdx = 1, 1
    self.subPosTrainSet, self.subNegTrainSet = subPosTrainSet, subNegTrainSet
end
]=]
...
function SingleDataSet:getNextBatch(batch_size)
	self.batch_size = batch_size or 18
    local numPos = #self.subPosTrainSet 
    local numNeg = #self.subNegTrainSet
    local numPosBatch = self.batch_size / 3    -- num of positive samples for each batch
    local numNegBatch = self.batch_size - numPosBatch
	...
	local limit = self.posIdx + numPosBatch - 1
	for i = self.posIdx, limit  do	
        local vid = self.subPosTrainSet[self.posRandIdx[i]]
        local vidName = vid['name']
        local cutName = self:cut_name(vidName)
        table.insert(nextBatch, self.c3dFile:read('v_' .. cutName .. '/c3d_features'):all())
        table.insert(segment_targets, vid["gt_segments"])
        table.insert(label_targets, vid['gt_labels'])
	end
	local limit = self.negIdx + numNegBatch - 1
	for i = self.negIdx, limit  do	
        local vid = self.subNegTrainSet[self.negRandIdx[i]]
        local vidName = vid['name']
        local cutName = self:cut_name(vidName)
        table.insert(nextBatch, self.c3dFile:read('v_' .. cutName .. '/c3d_features'):all())
        table.insert(segment_targets, vid["gt_segments"])
