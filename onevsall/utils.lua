...
function utils.get_gts(locations, cls_target, reg_target)
    local batch_size = #cls_target
    local gt_cls_target = torch.Tensor(batch_size, 1)
    local gt_reg_target = torch.Tensor(batch_size, 2)

    for i = 1, batch_size do
        local loc = locations[i]
        local gts = reg_target[i]
        local dists = utils.segment_distance(gts, loc)
        local min_dis, min_disid = torch.min(dists, 1)
        gt_reg_target[i] = gts[min_disid[1]]
        gt_cls_target[i] = cls_target[i][min_disid[1]]
    end
    return gt_cls_target, gt_reg_target
end

function utils.segment_distance(gts, loc)
    local num_gt = gts:size(1)
    local dists = torch.Tensor(num_gt)
    for i = 1, num_gt do
        dists[i] = math.min(math.abs(gts[i][1] - loc), math.abs(gts[i][2] - loc))
    end
    return dists
end
